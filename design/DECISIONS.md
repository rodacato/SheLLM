...
**Status:** 20 entries · 17 resolved · 3 open · 0 🔴 high-impact · 0 🐞 unfiled bugs.
...
**The auto-refresh branch renumbered itself on 2026-09-23, and its commit messages did not.** It
opened `D64` and `D65` while the OSS-audit branch was in flight; that branch merged first and took
`D64` for the Keys sample-data finding, so these became **`D65`** (the band-2 grid drift) and
**`D66`** (the read stamp), with `D67` following (that third entry). The rule from the D30/D31 collision applies
unchanged: the range already merged and cited wins, the unmerged one moves. Commits `f7bc22e` and
`ae174c9` say D64, D65 and D66 in their messages and mean D65, D66 and D67.
...
| **D63** ✅ | The Keys table drew a key prefix the code cannot produce. Three rows read `sk-shellm-a1b2...`; `generateKey()` returns `'shellm-' + randomBytes(16).toString('hex')` and `key_prefix` is its first **8** characters, so the cell renders `shellm-a...` — the `sk-` was invented and the visible part was four times too long. | [`clients.js:44`](../src/db/clients.js#L44) and `KEY_PREFIX_LEN = 8` ([`clients.js:5`](../src/db/clients.js#L5)); the cell is `key.key_prefix + '...'` in [`keys.html`](../src/admin/views/pages/keys.html). No code anywhere in the repository emits `sk-`. | ✅ **Closed 2026-09-23.** Six cells across two artboards now read `shellm-a...`, `shellm-c...`, `shellm-e...`. It had shipped: this export is the source of the landing page's `api-keys.png` and of a README figure, so an invented key format was the one a stranger saw first. It survived because `sk-` is what every other vendor's key looks like — a plausible string is exactly the kind a review reads past, which is the argument for deriving sample data from the generator rather than from memory. |
| **D64** ✅ | Sample data in the Keys surface names another of the maintainer's private projects. The description placeholder read `e.g. Production key for Stockerly`, and the flow's two key rows read `Key mi-feria`. Neither name means anything to a stranger, and the placeholder ships in a public repo and in the published screenshots. | Code: [`keys.html:250`](../src/admin/views/pages/keys.html#L250), changed 2026-09-23 to `e.g. Production key for my blog pipeline`. Canvas: two nodes in [`flows/admin.pen`](flows/admin.pen) still read `mi-feria` | ✅ **Closed 2026-09-23.** Six nodes across three artboards — `Admin / API Keys / Default`, `Key created` and `Create` — now read `blog-pipeline`, a name derived from the audience [`docs/AUDIENCE.md`](../docs/AUDIENCE.md) describes rather than from the maintainer's own projects, and matching the vocabulary the code already set with `e.g. my-app` and `e.g. Production key for my blog pipeline`. It is also the row scoped to a browser origin, which makes the sample coherent: a page on GitHub Pages is exactly the caller that needs one. `grep -c mi-feria` on the file is 0. The count grew between the finding and its close — it was four occurrences in two artboards when D64 was written and six in three by the time it was worked, because `API Keys / Create` was drawn in the meantime as a copy of `Default` and inherited the sample data. A finding against duplicated sample data gets more expensive every time someone copies the artboard, which is the argument for closing one promptly rather than letting it wait for a convenient pass. Key prefixes were not touched: D63 derived them from the generator and they still read `shellm-a...`. Was: ⏳ **Code led; the canvas has not followed.** The placeholder was changed in the OSS audit pass, which had no Pencil session, so the flow and the code now disagree on this one string — recorded here rather than left to be re-found. The `.pen` fix is two node edits and wants the next design session. Same root cause as D63: sample data invented from the maintainer's own context rather than derived from the product's audience, which [`docs/AUDIENCE.md`](../docs/AUDIENCE.md) describes as apps like a blog pipeline or a cron job. |

| **D65** ✅ | Band 2 was 120 px off the grid from its ninth screen on. `System / Unreachable` sat at x 13120 where the gutter puts it at 13240, touching `Playground / No provider` with no gap at all, and `API Keys / Key created` inherited the shift at 14680. | Found by the identity probe at the top of the auto-refresh session rather than by looking: every other pair in the band is 1560 apart (1440 plus the 120 `grid-gutter` declares) and those two were 1440. | ✅ **Closed 2026-09-23**, two `Update` calls, band 2 now steps 1560 the whole way. Worth an entry for how it was found — the probe prints every artboard's x and the arithmetic is one subtraction, while on the canvas two boards 120 px apart and two boards flush against each other both read as "next to". Both were re-exported and came back **byte-identical**, which is the point: position is not rendering, so no screenshot review could ever have caught this. |

| **D66** ⏳ | The `read HH:MM:SS` clock does not say what it appears to say. It reports the last time **any** admin fetch succeeded, not the last time *this page's* data was read: `reportRead` writes it from every `apiFetch` ([`app.js:28`](../src/admin/public/js/app.js#L28)), and the health poll runs every 30 s on every page ([`app.js:251`](../src/admin/public/js/app.js#L251)). | Harmless today, because every page that shows the clock also re-reads itself at 30 s, so the two are never far apart. The auto-refresh proposal breaks that on purpose: with Overview switched off, the footer and the window line keep advancing from the health poll while every figure above them is frozen — the one element whose job is to say how stale the page is would be the only thing on it still moving. | ⏳ Open, and it is a decision for the implementation rather than a defect to fix first. Three ways out: scope the clock to the page's own read, leave it global and let the control carry the page's freshness instead, or show both and name them. Drawing the proposal is what raised it; the band deliberately encodes no answer. |

| **D67** ⏳ | The admin has no legible disabled treatment for **text**, and the one it has reads as ordinary secondary copy. `text-outline/30` — what the Logs refresh button uses while loading ([`logs.html:52`](../src/admin/views/pages/logs.html#L52)) — resolves to roughly `#3b4346` on `surface-container`, about **1.75:1**. Fine for a glyph whose only job is to look inert; unusable for a value someone still has to read. The nearest token, `outline-variant` `#3b494c`, measures the same. | Measured 2026-09-23 while drawing the auto-refresh held state, where the interval value must stay readable *because* it is the setting that is not currently running. `outline` `#849397` on `surface-container` is **5.15:1** and passes AA, but it is also the default colour of every secondary string on the page, so on its own it signals nothing — confirmed by exporting the board and being unable to tell the two states apart. | ⏳ Open as a kit question, not a bug: nothing shipped is wrong, and WCAG exempts disabled controls anyway. What is missing is a token between `outline` and `outline-variant` for "this control is legible but not currently doing anything". The proposal band works around it with a `pause` glyph rather than a colour, which is arguably the better answer and makes the token optional. Worth deciding before a second surface needs the same state. |
...
| **D62** ✅ 🐞 | The Request Logs filter row pushed the page sideways. A native `<select>` is as wide as its widest `<option>` and a flex item defaults to `min-width: auto`, so one long model name in the Model filter widened the row past `<main>` — which carries no `max-width` — and the whole page scrolled, cutting the title off at the left edge. | Reported by Adrian from the running instance, with a screenshot: four unbounded selects, the result count and three buttons on one `flex flex-wrap` row. The canvas could never have shown it — a drawn select has no intrinsic width, and `admin.pen` had them at 145 px looking correct throughout. | ✅ **Fixed 2026-09-23.** Every filter carries `w-44`, and the result count and the three actions moved to their own row beneath them, which is also the row that had no space for them before. [`logs-toolbar.test.js`](../test/admin/logs-toolbar.test.js) fails if any filter loses its width class — the property is what stops the data sizing the layout, so that is what is pinned rather than a screenshot. |
...
**Next free number: D68.** D15–D20, D43–D49 and D61–D67 are this file's own and are above; D50–D60
are spent by the landing audit.
