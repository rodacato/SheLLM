...
**Status:** 9 entries · 4 resolved · 5 open · 0 🔴 high-impact · 0 🐞 unfiled bugs.
...
| **D31** | `manifest.webmanifest` re-types two palette colours, and a JSON file cannot read a custom property. One of them is also the wrong colour: `background_color` is `#1a1e21` (`surface-shell`, the sidebar) while the body the splash screen resolves into is `surface` `#101417`. | [`manifest.webmanifest:9-10`](../src/admin/public/manifest.webmanifest#L9). `theme_color` matches `index.html`'s meta and is correct; `background_color` matches nothing the user sees full-bleed | ⏳ D19 removed every retyped literal it could reach and this is the one it structurally cannot. Recommend `background_color: "#101417"` and a line in the neighbourhood naming the two tokens both values mirror, so the next palette change does not miss the file the test cannot police. Logged as P10 in [`docs/PWA-AUDIT.md`](../docs/PWA-AUDIT.md). |
