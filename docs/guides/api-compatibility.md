...
**1M context** is on by default: a named model the CLI lists a 1M variant for (`claude-sonnet`, `claude-opus`, `claude-fable` on the account this was written on) runs as the CLI's `<alias>[1m]`, and `/v1/models` shows its `context_window_1m`. With `SHELLM_CLAUDE_LONG_CONTEXT=false` it is asked for the way the Anthropic API asks for it: the same model id plus the header `anthropic-beta: context-1m-<date>` (any date), on `/v1/messages` or `/v1/chat/completions`. `claude-haiku` and the bare `claude` run at their normal window either way.
