...
<style>
  :root { color-scheme: dark; --bg:#1a1e21; --panel:#242a2e; --line:#38424a; --text:#f9f9f7; --accent:#03e3ff; --error:#ff6b6b; }
  * { box-sizing: border-box; }
  ...
  p { margin:0 0 1.5rem; font-size:.8rem; color:#9bb; }
  label { display:block; font-size:.7rem; text-transform:uppercase; letter-spacing:.1em; margin:1rem 0 .35rem; color:#9bb; }
  input { width:100%; padding:.6rem .7rem; background:var(--bg); border:1px solid var(--line); color:var(--text); font:inherit; }
  input:focus { outline:2px solid var(--accent); outline-offset:-2px; }
  button { width:100%; margin-top:1.5rem; padding:.7rem; background:var(--accent); color:#05191d; border:0;
           font:inherit; font-weight:700; text-transform:uppercase; letter-spacing:.08em; cursor:pointer; }
  .error { margin:1rem 0 0; padding:.6rem .7rem; border-left:3px solid var(--error); background:#2e2226; color:var(--error); font-size:.8rem; }
</style>
</head>
<body>
<form method="post" action="/admin/login">
  <h1>SheLLM<span>_</span></h1>
  <p>Admin access</p>
